...
final class AdminCandidateTest extends CIUnitTestCase
{
    ...
    {
        $this->submit('admin/candidates/2', $this->fields([
            'nomor_urut'   => '2',
        ...
        $ballot = $this->withSession(['user_type' => 'student', 'student_id' => 1, 'isLoggedIn' => true])->get('student/vote');
    ...
    {
        $this->submit('admin/candidates/1', $this->fields(['nomor_urut' => '1', 'nama_ketua' => 'Arka Wibisana', 'nama_wakil' => 'Naya Kirana']), [
        ...
        $this->submit('admin/candidates/1', $this->fields([
            'nomor_urut'   => '1',
    ...
    public function testInvalidFieldsAndFilesAreRejectedWithoutSavingAnything(): void
    {
        ...
        $this->submit('admin/candidates', $this->fields(['nomor_urut' => '1', 'theme_accent' => 'linear-gradient(red, blue)']))
            ->assertRedirectTo(site_url('admin/candidates/new'));
        ...
        $this->submit('admin/candidates', $this->fields(), [
        ...
        $form = $this->withSession()->get('admin/candidates/new');
    ...
    {
        $this->submit('admin/candidates/3', $this->fields([
            'nomor_urut'  => '3',
            'id'          => '99',
    ...
    public function testCandidateWithVotesCannotBeDeletedButCanBeDeactivated(): void
    {
        ...
        $this->submit('admin/candidates/1/delete', [])->assertRedirectTo(site_url('admin/candidates'));
        $this->assertStringContainsString('tidak dapat dihapus', (string) session('error'));
        $this->seeInDatabase('candidates', ['id' => 1]);

        $this->submit('admin/candidates/1', $this->fields(['nomor_urut' => '1', 'status_aktif' => '0']))
            ->assertRedirectTo(site_url('admin/candidates'));
        $this->seeInDatabase('candidates', ['id' => 1, 'status_aktif' => 0]);

        // Nonaktif: hilang dari surat suara, suara lama tetap dihitung di analitik.
        $ballot = $this->withSession(['user_type' => 'student', 'student_id' => 2, 'isLoggedIn' => true])->get('student/vote');
        $ballot->assertDontSee('Coblos Pasangan 01');
        $live = json_decode($this->withSession($this->admin())->get('admin/live-count')->getJSON(), true);
    ...
    public function testCandidateWithoutVotesCanBeDeletedWithItsFiles(): void
    {
        ...
        $this->submit('admin/candidates/' . $row['id'] . '/delete', [])->assertRedirectTo(site_url('admin/candidates'));
    ...
    {
        $result = $this->withSession($this->admin())->get('admin/candidates/2/preview');
    ...
    {
        $result = $this->withSession($this->admin())->get('admin/candidates/1/edit');
