...
final class AdminImportTest extends CIUnitTestCase
{
    ...
    private function upload(string $role, string $path, string $name = 'data.xlsx')
    {
        ...
        return $this->withSession($this->admin())->post('admin/' . $role . '/import', [csrf_token() => csrf_hash()]);
    }

    private function tokenFrom($result): string
    {
        $location = $result->response()->getHeaderLine('Location');
        $this->assertMatchesRegularExpression('#/import/preview/([a-f0-9]{32})$#', $location);
    ...
    {
        $result = $this->withSession($this->admin())->get('admin/students/import/template');
        // Header unduhan dibentuk saat respons dikirim; di test dibentuk manual.
        $result->response()->buildHeaders();

        $result->assertStatus(200);
        $this->assertStringContainsString('student-import-template.xlsx', $result->response()->getHeaderLine('Content-Disposition'));
        ...
        $teacher = $this->withSession($this->admin())->get('admin/teachers/import/template');
        $teacher->response()->buildHeaders();
        $this->assertStringContainsString('teacher-import-template.xlsx', $teacher->response()->getHeaderLine('Content-Disposition'));
    }

    public function testStudentImportFlowPreviewCommitResultAndAudit(): void
    {
        $upload = $this->upload('students', SpreadsheetFactory::students([
        ...
        $preview = $this->withSession($this->admin())->get('admin/students/import/preview/' . $token);
        ...
        $all = $this->withSession($this->admin())->get('admin/students/import/preview/' . $token . '?show=all');
        ...
        $commit = $this->withSession($this->admin())->post('admin/students/import/commit', [
            csrf_token()   => csrf_hash(),
            'token'        => $token,
            'confirm_skip' => '1',
        ]);
        $commit->assertRedirectTo(site_url('admin/students/import/result'));
        ...
        $result = $this->withSession()->get('admin/students/import/result');
        $result->assertStatus(200);
        $result->assertSee('Impor selesai');
        $result->assertSee('Ditambahkan');

        // Token sekali pakai.
        $this->withSession($this->admin())->get('admin/students/import/preview/' . $token)
            ->assertRedirectTo(site_url('admin/students/import'));
    }

    public function testTeacherImportFlow(): void
    {
        $token = $this->tokenFrom($this->upload('teachers', SpreadsheetFactory::teachers([
            [1, '198501012010011001', 'Guru Baru, S.Pd.', '01011985'],
            [2, '000000000000000004', 'Siti Nur Aini, M.Pd.', '01061992'],
        ])));

        $this->withSession($this->admin())->post('admin/teachers/import/commit', [csrf_token() => csrf_hash(), 'token' => $token])
            ->assertRedirectTo(site_url('admin/teachers/import/result'));
    ...
    {
        // Header salah.
        $this->upload('students', SpreadsheetFactory::write([['no', 'NIP', 'nama', 'kodeunik'], [1, '1985', 'X', '01011985']]))
            ->assertRedirectTo(site_url('admin/students/import'));
        ...
        $this->upload('students', $csv, 'siswa.csv')->assertRedirectTo(site_url('admin/students/import'));
        $this->assertStringContainsString('Format file harus Excel .xlsx', (string) session('error'));

        // Tanpa file.
        UploadFixture::reset();
        $this->withSession($this->admin())->post('admin/students/import', [csrf_token() => csrf_hash()])
            ->assertRedirectTo(site_url('admin/students/import'));
    ...
    public function testPreviewTokenIsBoundToTheUploadingAdmin(): void
    {
        ...
        $token = $this->tokenFrom($this->upload('students', SpreadsheetFactory::students([
            [1, '0012345678', 'Siswa A', 'L', '8C', 1, '01032012'],
        ])));

        $this->withSession($this->admin(2))->get('admin/students/import/preview/' . $token)
            ->assertRedirectTo(site_url('admin/students/import'));
        $this->withSession($this->admin(2))->post('admin/students/import/commit', [csrf_token() => csrf_hash(), 'token' => $token])
            ->assertRedirectTo(site_url('admin/students/import'));

        // Token siswa tidak berlaku untuk impor guru.
        $this->withSession($this->admin())->get('admin/teachers/import/preview/' . $token)
            ->assertRedirectTo(site_url('admin/teachers/import'));
    ...
    {
        $result = $this->withSession($this->admin())->get('admin/students/import');

        $result->assertStatus(200);
        $result->assertSee('student-import-template.xlsx');
        $result->assertSee('href="' . site_url('admin/students/import/template') . '"');
